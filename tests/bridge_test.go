...
func TestStdClient_SendRequest(t *testing.T) {
	t.Parallel()
...
func TestStdClient_SyncFields_PreservesProperties(t *testing.T) {
	t.Parallel()
...
func TestStdClient_VariousStatusCodes(t *testing.T) {
	t.Parallel()
...
func TestWithContextModifier_ConfigureContext(t *testing.T) {
	t.Parallel()
...
func TestContextModifiers_Retrieve(t *testing.T) {
	t.Parallel()
...
func TestNewStdClient_DefaultSetup(t *testing.T) {
	t.Parallel()
...
func TestTransport_RoundTrip_NilURL(t *testing.T) {
	t.Parallel()
	...
	tests := []struct {
		...
		{
			name:        "nil_url_closes_body_on_failure",
			hasBody:     true,
			expectedErr: aoni.ErrNilURL,
		},
		{
			name:        "nil_url_no_body",
			hasBody:     false,
			expectedErr: aoni.ErrNilURL,
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			var mockBody *mockReadWriteCloser

			if tt.hasBody {
				mockBody = &mockReadWriteCloser{}
				req.Body = mockBody
			}
			...
			assert.ErrorIs(t, urlErr.Err, tt.expectedErr)

			if tt.hasBody {
				assert.True(t, mockBody.closed.Load(), "expected request body to be closed")
			}
		})
	}
}

func TestTransport_RoundTrip_RequestFailure_ClosesBody(t *testing.T) {
	t.Parallel()
	...
	tests := []struct {
		...
		{
			name:    "with_body_closes_body_on_failure",
			hasBody: true,
		},
		{
			name:    "without_body_completes",
			hasBody: false,
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.hasBody {
				assert.True(t, mockBody.closed.Load(), "expected request body to be closed on error pathways")
			}
		})
	}
}

func TestTransport_RoundTrip_BaseURL_Rewriting(t *testing.T) {
	t.Parallel()
	...
	tests := []struct {
		...
		{
			name:          "relative_url_without_host_untouched",
			requestURL:    "/relative_path",
			shouldRewrite: false,
		},
		{
			name:           "protocol_relative_url_fallback_to_https",
			requestURL:     "//example.com/test",
			expectedScheme: "https",
			expectedHost:   "example.com",
			shouldRewrite:  true,
		},
		{
			name:           "absolute_http_url_keeps_scheme",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			req, err := http.NewRequestWithContext(t.Context(), http.MethodGet, tt.requestURL, nil)
			require.NoError(t, err)
			...
			if tt.shouldRewrite {
				...
				assert.Equal(t, tt.expectedScheme, capturedURL.Scheme)
				assert.Equal(t, tt.expectedHost, capturedURL.Host)
			} else {
				assert.Empty(t, capturedURL.Host)
			}
		})
	}
}

func TestTransport_RoundTrip_BeforeRoundTripHook(t *testing.T) {
	t.Parallel()
...
func TestNewStdClient_AutoRestoreGetBody(t *testing.T) {
	t.Parallel()
...
func TestNewStdClient_ParallelRequests(t *testing.T) {
	t.Parallel()
...
func TestNewStdClient_Resilience(t *testing.T) {
	t.Parallel()
