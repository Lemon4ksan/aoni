...
func newBillingAPI(doer any, opts ...aoni.ClientOption) *billingAPIClient {
	...
	if d, ok := doer.(aoni.RequestDoer); ok {
		targetReq = request.AsRequester(aoni.Configure(d, append([]aoni.ClientOption{option.WithBaseURL("https://api.fintech.com/v1")}, baseOpts...)...))
	} else if req, ok := doer.(request.Requester); ok {
		targetReq = request.AsRequester(aoni.Configure(req, append([]aoni.ClientOption{option.WithBaseURL("https://api.fintech.com/v1")}, baseOpts...)...))
		...
		targetReq = request.AsRequester(aoni.Configure(fast.NewClient(), append([]aoni.ClientOption{option.WithBaseURL("https://api.fintech.com/v1")}, baseOpts...)...))
	}
