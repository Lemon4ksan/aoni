...
var (
	...
	ErrVerifierLength = errors.New("aoni/pkce: code verifier length must be between 43 and 128 characters")

	// ErrInvalidMethod is returned when an unsupported code challenge method is requested.
	ErrInvalidMethod = errors.New("aoni/pkce: unsupported code challenge method (use S256)")
)
...
func GenerateVerifier(length int) (string, error) {
	...
	if _, err := rand.Read(buf); err != nil {
		return "", fmt.Errorf("aoni/pkce: failed to generate random bytes: %w", err)
	}
