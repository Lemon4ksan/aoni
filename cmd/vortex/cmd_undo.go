...
func (c *CmdUndo) Usage() string {
	return "vortex undo [op-id] [flags]"
}
...
func (c *CmdUndo) Run(ctx context.Context, args []string, stdout, stderr io.Writer) error {
	...
	fs.Usage = func() {
		fmt.Fprintf(stderr, "vortex undo — AST Operation Rollback & Revert Tool\n\n")
		fmt.Fprintf(stderr, "Usage:\n")
		fmt.Fprintf(stderr, "  vortex undo [op-id]\n\n")
		fmt.Fprintf(stderr, "Examples:\n")
		fmt.Fprintf(stderr, "  vortex undo         # Revert the latest modification\n")
		fmt.Fprintf(stderr, "  vortex undo op-a1b2 # Revert a specific operation\n")
		fmt.Fprintf(stderr, "  vortex history      # View list of reversible operations\n\n")
