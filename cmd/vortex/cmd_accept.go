...
func (c *CmdAccept) Usage() string { return "vortex accept <branch-name|git-ref>" }
...
func (c *CmdAccept) Run(ctx context.Context, args []string, stdout, stderr io.Writer) error {
	...
	fs.Usage = func() {
		fmt.Fprintf(stderr, "vortex accept — Semantic Proposal Reconciler & Merger\n\n")
		fmt.Fprintf(stderr, "Usage:\n")
		fmt.Fprintf(stderr, "  vortex accept [-no-gen] <branch-name|git-ref>\n\n")
	...
	positional := fs.Args()
	if len(positional) == 0 {
		return errors.New("missing proposal branch argument (e.g. `vortex accept feat/add-search`)")
	}
	...
	fmt.Fprintf(stdout, "⚡ [vortex accept] Merging Proposal from %q into local master...\n\n", targetRef)
