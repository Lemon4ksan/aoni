...
func (c *CmdEnv) Run(ctx context.Context, args []string, stdout, stderr io.Writer) error {
	...
	StringVar(fs, &outFile, "out", "o", ".env.example", "Output file for environment variables (use '-' for stdout)")
	BoolVar(fs, &fill, "fill", "f", false, "Pre-fill variables from local .vortex/cache/secrets.json vault")
	StringVar(fs, &dir, "dir", "d", "", "Target workspace directory (default: current root)")
