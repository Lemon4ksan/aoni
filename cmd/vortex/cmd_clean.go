...
func (c *CmdClean) Run(ctx context.Context, args []string, stdout, stderr io.Writer) error {
	fs := flag.NewFlagSet("clean", flag.ContinueOnError)
	fs.SetOutput(stderr)

	var (
		allFlag    = fs.Bool("all", false, "Also remove primary generated API clients (*.gen.go)")
		allA       = fs.Bool("a", false, "Alias for --all")
		dryRunFlag = fs.Bool("dry-run", false, "Preview files to be deleted without removing them")
		dryRunN    = fs.Bool("n", false, "Alias for --dry-run")
