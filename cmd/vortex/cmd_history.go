...
func (c *CmdHistory) Usage() string {
	return "vortex history [--json] [flags]"
}
...
func (c *CmdHistory) Run(ctx context.Context, args []string, stdout, stderr io.Writer) error {
	...
	fs.Usage = func() {
		fmt.Fprintf(stderr, "vortex history — AST Operation Journal & History\n\n")
		fmt.Fprintf(stderr, "Usage:\n")
		fmt.Fprintf(stderr, "  vortex history [--json]\n\n")
		fmt.Fprintf(stderr, "Examples:\n")
		fmt.Fprintf(stderr, "  vortex history\n")
		fmt.Fprintf(stderr, "  vortex undo         # Revert the latest operation\n")
		fmt.Fprintf(stderr, "  vortex undo op-a1b2 # Revert a specific operation\n\n")
	...
	fmt.Fprintf(stdout, "\nTip: Run `vortex undo` to revert the latest operation.\n")
