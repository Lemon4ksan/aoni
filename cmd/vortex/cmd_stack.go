...
func (c *CmdStack) Usage() string {
	return "vortex stack <push|list|diff|pop|restore|clear> [flags] [args...]"
}

func (c *CmdStack) Run(ctx context.Context, args []string, stdout, stderr io.Writer) error {
	if len(args) == 0 {
		c.printUsage(stderr)
		return nil
	}
	...
	case "help", "-h", "--help":
		...
		return nil
	default:
		return fmt.Errorf("unknown stack subcommand %q. Run 'vortex stack --help' for available commands", subcmd)
	}
}
...
func (c *CmdStack) printUsage(w io.Writer) {
	fmt.Fprintf(w, "vortex stack — Snapshot comparison stack for step-by-step reverse engineering\n\n")
	fmt.Fprintf(w, "Usage:\n")
	fmt.Fprintf(w, "  vortex stack push [flags] [files...]      Capture new snapshot frame\n")
	fmt.Fprintf(w, "  vortex stack list [--json]                Display ASCII stack tree diagram\n")
	fmt.Fprintf(w, "  vortex stack diff [--cumulative] [flags]  Diff AST tuple/method evolution\n")
	fmt.Fprintf(w, "  vortex stack pop [--restore]              Pop top frame\n")
	fmt.Fprintf(w, "  vortex stack restore <frame|label|base>   Rollback to target frame\n")
	fmt.Fprintf(w, "  vortex stack clear                        Clear all frames\n\n")
	fmt.Fprintf(w, "Examples:\n")
	fmt.Fprintf(w, "  vortex stack push -label=\"raw-ingest\"\n")
	fmt.Fprintf(w, "  vortex stack push -label=\"deobf-models\" -tags=\"tuples\"\n")
	fmt.Fprintf(w, "  vortex stack diff                         # Diff top vs previous frame\n")
	fmt.Fprintf(w, "  vortex stack diff --cumulative            # Cumulative evolution vs base\n")
	fmt.Fprintf(w, "  vortex stack restore base                 # Jump back to base\n")
}
