...
func (c *CmdSource) Usage() string {
	return "vortex source [list|set|rm|fetch|diff|ping|sync] [contract] [source_url_or_path] [flags]"
}
...
func (c *CmdSource) Run(ctx context.Context, args []string, stdout, stderr io.Writer) error {
	...
	fs.Usage = func() {
		fmt.Fprintf(stderr, "vortex source — Manage upstream API specifications and remote schemas\n\n")
		fmt.Fprintf(stderr, "Usage:\n")
		fmt.Fprintf(
			stderr,
			"  vortex source [list]                                   List all configured upstream sources\n",
		)
		fmt.Fprintf(
			stderr,
			"  vortex source set <contract> <url_or_path> [--fetch]   Bind an upstream specification to contract\n",
		)
		fmt.Fprintf(
			stderr,
			"  vortex source rm <contract>                            Unbind upstream source from contract\n",
		)
		fmt.Fprintf(
			stderr,
			"  vortex source fetch [contract]                         Download remote specifications locally\n",
		)
		fmt.Fprintf(
			stderr,
			"  vortex source ping [contract]                          Verify reachability of remote endpoints\n",
		)
		fmt.Fprintf(
			stderr,
			"  vortex source diff [contract]                          Compare Go contract against upstream schema\n",
		)
		fmt.Fprintf(
			stderr,
			"  vortex source sync [contract]                          Fetch and re-generate API clients\n\n",
		)
...
func (c *CmdSource) runSet(
	...
	if contractName == "" || source == "" {
		return errors.New("usage: vortex source set <contract> <url_or_file>")
	}
...
func (c *CmdSource) runRm(stdout io.Writer, cfg *project.Config, contractName string) error {
	if contractName == "" {
		return errors.New("usage: vortex source rm <contract>")
	}
...
func (c *CmdSource) runDiff(ctx context.Context, stdout io.Writer, cfg *project.Config, target string) error {
	...
	if len(targets) == 0 {
		fmt.Fprintf(stdout, "No upstream sources configured to diff. Bind a source with `vortex source set`.\n")
		return nil
	}
