...
// Package framing implements binary frame serialization and deserialization.
package framing
...
var (
	...
	ErrPayloadTooLarge = errors.New("framing: frame payload exceeds maximum allowed size")

	// ErrTruncatedHeader is returned when the 5-byte frame header is truncated.
	ErrTruncatedHeader = errors.New("framing: truncated 5-byte frame header")

	// ErrTruncatedPayload is returned when the frame payload stream is truncated.
	ErrTruncatedPayload = errors.New("framing: truncated frame payload")
)
