...
func emitNestedTupleFieldUnmarshal(buf *bytes.Buffer, f ir.TupleFieldIR) {
	...
	fmt.Fprintf(buf, "%sif len(raw) > %d && len(raw[%d]) > 0 && string(raw[%d]) != \"null\" {\n", indent, path[0], path[0], path[0])
	...
	for i := 1; i < len(path); i++ {
		...
		fmt.Fprintf(buf, "%sif err := json.Unmarshal(%s, &%s); err == nil && len(%s) > %d && len(%s[%d]) > 0 && string(%s[%d]) != \"null\" {\n",
			indent, curVar, subVar, subVar, path[i], subVar, path[i], subVar, path[i])
