...
func (r *RingBuffer[T]) PopFront() T {
	if r.Empty() {
		panic("github.com/lemon4ksan/aoni/internal/quic/internal/utils/ringbuffer: pop from an empty queue")
	}
...
func (r *RingBuffer[T]) PeekFront() T {
	if r.Empty() {
		panic("github.com/lemon4ksan/aoni/internal/quic/internal/utils/ringbuffer: peek from an empty queue")
	}
