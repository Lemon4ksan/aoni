...
| :--- | :---: | :---: | :---: | :---: | :---: |
| **Задержка GET JSON (`ns/op`)** | 49 612 ns | 56 766 ns | 13 198 ns | **5 028 ns** | **В ~3.7 раза быстрее (Bridge) / в 11.3 раз (Native)** |
| **Задержка Client GET (`ns/op`)** | 6 899 ns | **6 236 ns** | **3 834 ns** | **577.4 ns** | **Быстрее stdlib net/http и чистейшего fasthttp** |
| **Память в куче (`B/op`)** | 6 898 B | **5 853 B** | **2 218 B** | **361 B** | **В ~1.18 раза легче (Standard) / в 26 раз (Native)** |
| **Аллокации (`allocs/op`)** | 78 allocs | **66 allocs** | **19 allocs** | **8 allocs** | **-12 аллокаций (Standard) / 8 аллокаций (Native)** |
| **Задержка HTTP/2 (`ns/op`)** | 74 088 ns | 74 088 ns | 72 665 ns | **72 665 ns** | **На 22% меньше памяти в H2 (7.2KB против 9.3KB)** |
| **Задержка HTTP/3 (`ns/op`)** | 127 206 ns | 127 206 ns | 126 307 ns | **126 307 ns** | **На 35% меньше памяти в QUIC (15.1KB против 23.4KB)** |
| **Параллельная задержка (`ns/op`)** | 11 307 ns | 9 534 ns | 1 940 ns | **577.4 ns** | **В ~3.4 раза быстрее (Bridge) / в 16.5 раз (Native)** |
| **Параллельная память и GC (`B / alloc`)** | 6 898 B / 78 | 5 853 B / 66 | 2 218 B / 19 | **0 B / 0 allocs** | **Ноль аллокаций в куче** |
| **Пиковая пропускная способность** | ~35k RPS | ~30k RPS | >80 000 RPS | **1 800 000+ RPS** | **Высоконагруженный I/O** |
...
> За счет повторного использования буферов через `sync.Pool` и нативного SIMD AVX2 ускорения (`simd_amd64.s`), `aoni/fast` работает с **0 B/op и 0 allocs/op** в параллельном I/O. Стандартный движок `aoni.Client` переигрывает базовый stdlib `net/http` (5.8KB vs 6.8KB, 66 vs 78 аллокаций), а `aoni/fast` полностью изолирует рантайм Go от давления GC, обеспечивая плоскую наносекундную задержку и 1.8M+ RPS на одном сервере.

> [!NOTE]
> **Почему богатый возможностями `aoni` быстрее «наивного использования» `net/http` и `fasthttp`?**
> Обычный «наивный» код интеграции HTTP в Go регулярно повторно парсит URL-адреса, создает временные структуры `url.URL`, итераторы мап и небуферизованные слайсы байт в куче.
> `aoni` превосходит чистые вызовы благодаря глубокой геометрии памяти: предварительной компиляции URL-путей (`PreparedConfig`), пулированию объектов запросов в `sync.Pool`, предкомпилированным слайсам заголовков, инлайн-парсеру чисел (`ParseUintFast`) и прямому переносу указателей на горячем пути выполнения.
