...
| Метрика | Стандартная обертка (Resty) | `aoni` (Стандартный) | `aoni` + `fast.Bridge` | `aoni/fast` (Нативный) | Преимущество (`fast.Bridge` / `fast`) |
| :--- | :---: | :---: | :---: | :---: | :---: |
| **Задержка GET JSON (`ns/op`)** | 58 393 ns | 56 669 ns | 14 127 ns | **5 703 ns** | **В ~5 раз быстрее (Bridge) / в 10 раз (Native)** |
| **Потребление памяти (`B/op`)** | 9 113 B | 8 217 B | 2 671 B | **363 B** | **В ~3.4 раза легче (Bridge) / в 25 раз (Native)** |
| **Число аллокаций (`allocs/op`)** | 91 allocs | 82 allocs | 34 allocs | **8 allocs** | **В ~2.7 раза меньше (Bridge) / в 11 раз (Native)** |
...
| **Пиковая пропускная способность** | ~30k RPS | ~35k RPS | >70 000 RPS | **1 695 000+ RPS** | **Пиковая скорость кремния** |

> [!TIP]
> **100% совместимость с `net/http` с ускорением в 5 раз**: Подключая `fast.Bridge` к `aoni.Client`, стандартный код Go сохраняет полную совместимость с `net/http.Client`, получая **снижение задержки в 5 раз** (с ~58 мкс до 14.1 мкс). Для сырой многопоточной пропускной способности нативный `aoni/fast` работает на скорости **1.695M+ RPS** со 100% нулем аллокаций (**0 B/op**).
...
> **Хотите узнать спецификацию работы с процессором и памятью?**  
> Прочитайте документ [**CPU & Silicon Sympathy Specification**](docs/CPU_STACK.md), описывающий native PLAN9 AVX2 ассемблер, 2MB LargePages арены и наносекундный бюджет инструкций CPU.
