...
option / mod ──┼
               └──► fast.Client (2.14M+ RPS multi-core, нативный zero-alloc H1/H2/H3 движок)
```
...
* **Нативный `fast.Client`**: Используется, когда необходимы максимальная пропускная способность, 100% покрытие Scoped Borrow и нулевые аллокации (0 B/op).
...
При высокой параллельной нагрузке на всех ядрах процессора аллокатор памяти Go (`mcache`/`mcentral`) начинает испытывать конкуренцию за блокировки. Поскольку `aoni` выполняет **на 12 аллокаций меньше** за запрос, чем стандартный `net/http` (66 против 78 аллокаций), он намного лучше масштабируется, обеспечивая **задержку на 10% ниже** в стандартном режиме и **в 3.4–36 раз более высокую производительность** в режимах Bridge и Native.

| Метрика | Стандартный `net/http` | `aoni` (Стандартный) | `aoni` + `fast.Bridge` | `aoni/fast` (Нативный) | Разница производительности |
| :--- | :---: | :---: | :---: | :---: | :---: |
| **Анмаршалинг GET JSON Zero-Copy (`JSONNoCopy`)** | 127 905 ns | 54 824 ns | **10 948 ns** | **3 542 ns** | **⚡ В 36.1 раз быстрее (3.5 мкс, 51 B, 2 allocs)** |
| **Анмаршалинг GET JSON Standard (`GetTo[T]`)** | 127 905 ns | 54 824 ns | **10 948 ns** | **5 407 ns** | **⚡ В 23.6 раз быстрее (Native) / в 11.6 раз (Bridge)** |
| **Чистое исполнение (`c.Request`)** | 7 002 ns | **6 167 ns** | **5 500 ns** | **4 011 ns** | **⚡ В 1.7 раза быстрее / в 2.7 раз меньше RAM** |
| **Загрузка Multipart Form** | 273 999 ns | — | — | **102 539 ns** | **⚡ В 2.7 раза быстрее / в 4.5 раз меньше RAM (120KB vs 546KB)** |
| **Память в куче (`B/op`)** | 6 990 B | **6 165 B** | **5 928 B** | **51 B – 362 B** | **⚡ До 140 раз легче потребление памяти** |
| **Аллокации (`allocs/op`)** | 78 allocs | **68 allocs** | **48 allocs** | **2 – 8 allocs** | **⚡ -76 аллокаций (Zero-Copy) / -70 аллокаций (Native)** |
| **Задержка HTTP/2 (`ns/op`)** | 80 979 ns | 80 979 ns | **67 441 ns** | **67 441 ns** | **⚡ В 1.95 раза меньше памяти в H2 (5.0KB против 9.8KB)** |
...
| **Параллельная задержка под нагрузкой** | 7 002 ns | 6 167 ns | **1 940 ns** | **578.3 ns** | **⚡ В 12.4x – 15.8x быстрее (0 B / 0 allocs)** |
| **Пиковая пропускная способность (1 ядро)** | ~142k RPS | ~162k RPS | ~185k RPS | **~243 000+ RPS** | **⚡ Прирост в 1.7x на одно ядро** |
| **Пиковая пропускная способность (12 ядер)** | ~140k RPS | ~162k RPS | >550 000 RPS | **1 910 000+ RPS (2.05M+ пик)** | **⚡ В 13.6 раз выше параллельный RPS** |
