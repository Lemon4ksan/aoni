...
| :--- | :---: | :---: | :---: | :---: | :---: |
| **Задержка GET JSON (`ns/op`)** | 49 936 ns | 58 963 ns | 13 926 ns | **5 594 ns** | **В ~3.5 раза быстрее (Bridge) / в 10.5 раз (Native)** |
| **Задержка Client GET (`ns/op`)** | 5 906 ns | 6 091 ns | 5 675 ns | **3 868 ns** | **На уровне чистейшего fasthttp (3 770 ns)** |
| **Память в куче (`B/op`)** | 6 803 B | 9 548 B | 6 757 B | **361 B** | **В ~1.4 раза легче (Bridge) / в 26 раз (Native)** |
| **Аллокации (`allocs/op`)** | 76 allocs | 89 allocs | 68 allocs | **8 allocs** | **В ~1.15 раза меньше (Bridge) / в 11 раз (Native)** |
| **Задержка HTTP/2 (`ns/op`)** | 75 337 ns | 75 337 ns | 73 353 ns | **73 353 ns** | **На 22% меньше памяти в H2 (7.2KB против 9.3KB)** |
| **Задержка HTTP/3 (`ns/op`)** | 130 231 ns | 130 231 ns | 131 989 ns | **131 989 ns** | **На 35% меньше памяти в QUIC (15.1KB против 23.4KB)** |
| **Параллельная задержка (`ns/op`)** | 11 307 ns | 9 534 ns | 1 940 ns | **588.9 ns** | **В ~3.2 раза быстрее (Bridge) / в 16 раз (Native)** |
| **Параллельная память и GC (`B / alloc`)** | 6 803 B / 76 | 9 548 B / 89 | 6 757 B / 68 | **0 B / 0 allocs** | **Ноль аллокаций в куче** |
| **Пиковая пропускная способность** | ~35k RPS | ~30k RPS | >80 000 RPS | **1,695,000+ RPS** | **Высоконагруженный I/O** |
