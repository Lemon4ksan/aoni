...
| :--- | :---: | :---: | :---: | :---: | :---: |
| **Анмаршалинг GET JSON (`GetTo[T]`)** | 55 532 ns | 55 710 ns | **16 633 ns** | **6 283 ns** | **В ~3.3 раза быстрее (Bridge) / в 8.8 раз (Native)** |
| **Чистое исполнение (`c.Request`)** | 6 899 ns | **6 236 ns** | **3 834 ns** | **531.9 ns** | **В ~1.1 раза быстрее (Standard) / в 13 раз (Native)** |
| **Память в куче (`B/op`)** | 6 898 B | **5 853 B** | **2 218 B** | **362 B** | **В ~1.18 раза легче (Standard) / в 19 раз (Native)** |
| **Аллокации (`allocs/op`)** | 78 allocs | **66 allocs** | **19 allocs** | **8 allocs** | **-12 аллокаций (Standard) / 8 аллокаций (Native)** |
| **Задержка HTTP/2 (`ns/op`)** | 79 253 ns | 79 253 ns | 71 610 ns | **71 610 ns** | **В 2.08 раза меньше памяти в H2 (4.5KB против 9.4KB)** |
| **Задержка HTTP/3 (`ns/op`)** | 140 084 ns | 140 084 ns | 157 089 ns | **157 089 ns** | **На 50% меньше памяти в QUIC (11.6KB против 23.5KB)** |
| **Параллельная задержка (`ns/op`)** | 11 307 ns | 9 534 ns | 1 940 ns | **531.9 ns** | **В ~3.4 раза быстрее (Bridge) / в 18 раз (Native)** |
| **Параллельная память и GC (`B / alloc`)** | 6 898 B / 78 | 5 853 B / 66 | 2 218 B / 19 | **0 B / 0 allocs** | **Ноль аллокаций в куче** |
| **Пиковая пропускная способность** | ~35k RPS | ~30k RPS | >80 000 RPS | **2 146 709+ RPS** | **Высоконагруженный I/O** |
...
| :--- | :---: | :---: | :---: |
| **Чистый GET (`c.Request` + дренаж тела)** | 18 289 ns / 5 832 B / **67 allocs** | 18 543 ns / 6 157 B / **68 allocs** | +1 аллокация (+1.4% времени) |
| **Обобщённый GET + JSON decode (`request.GetTo[T]`)** | 20 087 ns / 6 751 B / **74 allocs** | 21 878 ns / 9 089 B / **76 allocs** | +2 аллокации (+8.9% времени) |
...
> Стандартный движок `aoni.Client` делает **на 12 аллокаций меньше** за запрос, чем `net/http` (66 против 78 аллокаций, 5.8KB vs 6.8KB), снижая нагрузку на аллокатор рантайма при многопоточном исполнении. В это же время `aoni/fast` (Native) повторно использует буферы через `sync.Pool`, задействует SIMD AVX2/BMI2 интринсики (`simd_amd64.s`), нетемпоральную потоковую запись и профильную оптимизацию PGO (`default.pgo`), работая с **0 B/op и 0 allocs/op** в параллельном I/O при плоской задержке **531.9 нс** и пропускной способности **2.14M+ RPS** на одном сервере.

> [!NOTE]
> **Объяснение разницы в 1 аллокацию в однопоточном режиме**
> При однопоточном исполнении (1 ядро, 0% параллелизма) сырой путь `aoni` показывает крошечную разницу в +1 аллокацию (+254 нс) по сравнению с чистым `net/http`. Это заранее собранная структура `*url.URL`, создаваемая из предкомпилированных компонентов `BaseURL` для решения относительных путей без вызова тяжелого `url.Parse`. В реальной сети (где задержка RTT ≥ 1 мс) эта разница в 0.25 мкс физически неизмерима, тогда как меньший объем используемой памяти `aoni` обеспечивает лучшее масштабирование на многоядерных серверах.
