...
option / mod ──┼
               └──► fast.Client (2.16M+ RPS multi-core, нативный zero-alloc H1/H2/H3 движок)
```
...
| **Параллельная задержка под нагрузкой** | 7 002 ns | 6 167 ns | **1 940 ns** | **462.9 ns** | **⚡ В 15.1x – 15.8x быстрее (0 B / 0 allocs)** |
| **Пиковая пропускная способность (1 ядро)** | ~142k RPS | ~162k RPS | ~185k RPS | **~243 000+ RPS** | **⚡ Прирост в 1.7x на одно ядро** |
| **Пиковая пропускная способность (12 ядер)** | ~140k RPS | ~162k RPS | >550 000 RPS | **2 160 293 RPS (2.16M+ пик)** | **⚡ В 15.4 раз выше параллельный RPS** |
