...
| **Параллельная задержка (`ns/op`)** | 11 307 ns | 9 534 ns | **1 940 ns** | **531.9 ns** | **В ~5.8 раза быстрее (Bridge) / в 21.2 раза (Native)** |
| **Параллельная память и GC (`B / alloc`)** | 6 898 B / 78 | 5 853 B / 66 | 2 218 B / 19 | **0 B / 0 allocs** | **Ноль аллокаций в куче** |
| **Пиковая пропускная способность** | ~35k RPS | ~30k RPS | >80 000 RPS | **2 000 000+ RPS** | **Экстремальный I/O** |
