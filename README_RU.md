...
| :--- | :---: | :---: | :---: | :---: | :---: |
| **Анмаршалинг GET JSON (`GetTo[T]`)** | 53 912 ns | 54 824 ns | **10 948 ns** | **4 887 ns** | **⚡ В 11.0 раз быстрее (Native) / в 4.9 раз (Bridge)** |
...
| **Память в куче (`B/op`)** | 6 990 B | **6 165 B** | **5 928 B** | **362 B** | **⚡ В 19.3 раз легче потребление памяти** |
| **Аллокации (`allocs/op`)** | 78 allocs | **68 allocs** | **48 allocs** | **8 allocs** | **⚡ -70 аллокаций (Native) / -30 аллокаций (Bridge)** |
...
| **Параллельная задержка под нагрузкой** | 7 002 ns | 6 167 ns | **1 940 ns** | **565.0 ns** | **⚡ В 12.4x – 15.8x быстрее (0 B / 0 allocs)** |
| **Пиковая пропускная способность (1 ядро)** | ~142k RPS | ~162k RPS | ~185k RPS | **~243 000+ RPS** | **⚡ Прирост в 1.7x на одно ядро** |
| **Пиковая пропускная способность (12 ядер)** | ~140k RPS | ~162k RPS | >550 000 RPS | **1 842 000+ RPS (2.14M+ пик)** | **⚡ В 13.4 раз выше параллельный RPS** |
