...
| **Чистое исполнение (`c.Request`)** | 6 899 ns | **6 236 ns** | **3 834 ns** | **577.4 ns** | **В ~1.1 раза быстрее (Standard) / в 12 раз (Native)** |
...
> Стандартный движок `aoni.Client` делает **на 12 аллокаций меньше** за запрос, чем `net/http` (66 против 78 аллокаций, 5.8KB vs 6.8KB), снижая нагрузку на аллокатор рантайма при многопоточном исполнении. В это же время `aoni/fast` (Native) повторно использует буферы через `sync.Pool` и задействует SIMD AVX2-ускорение (`simd_amd64.s`), работая с **0 B/op и 0 allocs/op** в параллельном I/O и обеспечивая плоскую наносекундную задержку и пропускную способность **1.8M+ RPS** на одном сервере.
