...
| **Задержка Client GET (`ns/op`)** | 5 807 ns | 6 091 ns | 5 475 ns | **3 809 ns** | **Быстрее чистейшего fasthttp (3 858 ns)** |
| **Память в куче (`B/op`)** | 6 898 B | 9 526 B | 6 754 B | **361 B** | **В ~1.4 раза легче (Bridge) / в 26 раз (Native)** |
| **Аллокации (`allocs/op`)** | 78 allocs | 89 allocs | 68 allocs | **8 allocs** | **В ~1.15 раза меньше (Bridge) / в 11 раз (Native)** |
...
| **Параллельная память и GC (`B / alloc`)** | 6 898 B / 78 | 9 526 B / 89 | 6 754 B / 68 | **0 B / 0 allocs** | **Ноль аллокаций в куче** |
| **Пиковая пропускная способность** | ~35k RPS | ~30k RPS | >80 000 RPS | **1,730,000+ RPS** | **Высоконагруженный I/O** |
